...
import { packingListsApi, warehousesApi, salesOrdersApi } from '../api';
...
export default function PackingListsPage() {
  ...
  const comparators = {
    ...
    'num-asc': (a, b) => (a.packingListNumber || '').localeCompare(b.packingListNumber || ''),
    'num-desc': (a, b) => (b.packingListNumber || '').localeCompare(a.packingListNumber || ''),
  };
    ...
    <div className="page-content">
      ...
      {showFilter && (
        <div style={{ marginBottom: 16, display: 'flex', gap: 8, flexWrap: 'wrap', alignItems: 'center' }}>
          ...
          >
            ...
            <option value="num-desc">Number (Z → A)</option>
          </select>
        </div>
      )}
      ...
      {loading ? (
        ...
        <Table
          ...
          columns={[
            ...
            { key: 'packingListNumber', label: 'Number', width: '160px', render: r => <span style={{ fontFamily: 'var(--font-mono)', fontWeight: 500 }}>{r.packingListNumber}</span> },
      ...
      {rowMenu && (
        <>
          ...
          <div style={{
            position: 'fixed', top: rowMenu.y + 4, left: rowMenu.x - 160, width: 160, zIndex: 1001,
            background: colors.surface, border: `1px solid ${colors.border}`, borderRadius: 10,
            padding: 6, boxShadow: '0 8px 24px rgba(0,0,0,0.12)',
          }}>
            {rowMenu.status === 'Draft' && (
              <MenuItem label="Mark as Ready" onClick={() => handleMarkReady(rowMenu.id)} />
            )}
            {(rowMenu.status === 'Draft' || rowMenu.status === 'Ready') && (
              <MenuItem label="Cancel" danger onClick={() => handleCancel(rowMenu.id)} />
            )}
          </div>
        </>
      )}
