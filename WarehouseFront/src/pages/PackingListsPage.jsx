...
import { Plus, MoreHorizontal, X, Search, ChevronRight, Package } from 'lucide-react';
import { colors } from '../theme/colors';
import { packingListsApi, warehousesApi, salesOrdersApi, palletsApi } from '../api';
...
export default function PackingListsPage() {
  ...
  const handleSelectRow = async (pl) => {
    if (selected?.id === pl.id) { setSelected(null); setPallets([]); return; }
    setSelected(pl);
  ...
  const toggleRowMenu = (e, r) => {
    ...
    setRowMenu(prev => prev?.id === r.id ? null : { id: r.id, x: rect.right, y: rect.bottom, status: r.status });
  };

  const toggleFilter = () => {
    setShowFilter(v => {
      if (v) { setQuery(''); setSortBy(''); setFilterStatus(''); }
      return !v;
    });
  };
  ...
  const filtered = packingLists.filter(pl => {
    const matchesQuery = !q || (pl.packingListNumber || '').toLowerCase().includes(q) || (pl.warehouseName || '').toLowerCase().includes(q);
  ...
  return (
    <div className="page-content">
      <PageHeader
        ...
        onExport={() => {
          const headers = ['Number', 'Sales Order', 'Warehouse', 'Date', 'Status'];
        ...
        action={<PrimaryButton icon={Plus} onClick={openCreate}>New Packing List</PrimaryButton>}
      />
      ...
      <div style={{ display: 'grid', gridTemplateColumns: selected ? '1fr 340px' : '1fr', gap: 16, alignItems: 'start' }}>
          ...
          {loading ? (
            ...
            <Table
              ...
              columns={[
                ...
                { key: 'packingListNumber', label: 'Number', width: '180px', render: r => (
                ...
                { key: 'salesOrderId', label: 'Sales Order', render: r => <span style={{ fontFamily: 'var(--font-mono)', fontSize: 12 }}>#{r.salesOrderId}</span> },
                { key: 'warehouseName', label: 'Warehouse', render: r => <span>{r.warehouseName || '—'}</span> },
                { key: 'createdAt', label: 'Date', width: '120px', render: r => (
                ...
                { key: 'action', label: '', width: '48px', render: r => (
                  <button onClick={e => toggleRowMenu(e, r)}
        ...
        {/* Detail Panel */}
        {selected && (
          <div style={{ background: colors.surface, border: `1px solid ${colors.border}`, borderRadius: 12, padding: 20, position: 'sticky', top: 100 }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'start', marginBottom: 16 }}>
              <div>
                <div style={{ fontSize: 11, color: colors.textMuted, fontFamily: 'var(--font-mono)', textTransform: 'uppercase', letterSpacing: '0.1em' }}>Packing List</div>
                <div style={{ fontSize: 16, fontWeight: 600, color: colors.text, fontFamily: 'var(--font-mono)', marginTop: 4 }}>{selected.packingListNumber}</div>
              </div>
              <button onClick={() => { setSelected(null); setPallets([]); }}
                style={{ all: 'unset', cursor: 'pointer', padding: 4, borderRadius: 4, color: colors.textMuted }}>
                <X size={16} />
              </button>
            </div>
            ...
            <div style={{ padding: 12, background: colors.bg, borderRadius: 8, marginBottom: 16 }}>
              <DetailRow label="Sales Order" value={`#${selected.salesOrderId}`} />
              <DetailRow label="Client" value={selected.clientName || '—'} />
              <DetailRow label="Warehouse" value={selected.warehouseName || '—'} />
              <DetailRow label="Status" value={selected.status} />
              <DetailRow label="Created" value={selected.createdAt ? new Date(selected.createdAt).toLocaleDateString() : '—'} />
              {selected.notes && <DetailRow label="Notes" value={selected.notes} />}
            </div>

            {/* Pallets */}
            <div style={{ fontSize: 11, color: colors.textMuted, fontFamily: 'var(--font-mono)', textTransform: 'uppercase', letterSpacing: '0.1em', marginBottom: 10 }}>
              Pallets
            </div>
            {palletsLoading ? (
              <div style={{ fontSize: 13, color: colors.textMuted, fontFamily: 'var(--font-mono)' }}>Loading...</div>
            ) : pallets.length === 0 ? (
              <div style={{ fontSize: 13, color: colors.textMuted, fontFamily: 'var(--font-sans)', padding: '12px 0' }}>No pallets found for this order.</div>
            ) : (
              <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                {pallets.map(p => (
                  <div key={p.id} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '8px 12px', background: colors.bg, borderRadius: 8, border: `1px solid ${colors.border}` }}>
                    ...
                    <div style={{ flex: 1 }}>
                      <div style={{ fontSize: 12, fontWeight: 500, fontFamily: 'var(--font-mono)', color: colors.text }}>{p.palletCode}</div>
                      <div style={{ fontSize: 11, color: colors.textMuted, fontFamily: 'var(--font-sans)', marginTop: 2 }}>{p.packingType}</div>
                    </div>
                  </div>
                ))}
              </div>
            ...
            <div style={{ marginTop: 16, display: 'flex', flexDirection: 'column', gap: 6 }}>
              {selected.status === 'Draft' && (
                <button onClick={() => handleMarkReady(selected.id)}
                  style={{ padding: '9px 0', border: `1px solid ${colors.border}`, borderRadius: 8, background: 'none', cursor: 'pointer', fontSize: 13, fontFamily: 'var(--font-sans)', color: colors.text }}>
                  Mark as Ready →
                </button>
              )}
              {(selected.status === 'Draft' || selected.status === 'Ready') && (
                <button onClick={() => handleCancel(selected.id)}
                  style={{ padding: '9px 0', border: 'none', borderRadius: 8, background: colors.dangerSoft, cursor: 'pointer', fontSize: 13, fontFamily: 'var(--font-sans)', color: colors.danger }}>
                  Cancel
                </button>
              )}
            </div>
      ...
      {rowMenu && (
        <>
          ...
          <div style={{ position: 'fixed', top: rowMenu.y + 4, left: rowMenu.x - 160, width: 160, zIndex: 1001, background: colors.surface, border: `1px solid ${colors.border}`, borderRadius: 10, padding: 6, boxShadow: '0 8px 24px rgba(0,0,0,0.12)' }}>
            {rowMenu.status === 'Draft' && <MenuItem label="Mark as Ready" onClick={() => handleMarkReady(rowMenu.id)} />}
            {(rowMenu.status === 'Draft' || rowMenu.status === 'Ready') && <MenuItem label="Cancel" danger onClick={() => handleCancel(rowMenu.id)} />}
          </div>
        </>
      )}
        ...
        <Modal title="New Packing List" onClose={() => setModalMode(null)}>
          <form onSubmit={handleSubmit}>
            <Field label="Sales Order">
              <select required style={inputStyle} value={form.salesOrderId} onChange={e => setForm(f => ({ ...f, salesOrderId: e.target.value }))}>
                <option value="" disabled>Select sales order...</option>
                {salesOrderOptions.map(so => (
                  <option key={so.id} value={so.id}>#{so.id} — {so.status}</option>
                ))}
              </select>
            </Field>
...
function DetailRow({ label, value }) {
  return (
    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '4px 0', borderBottom: `1px solid ${colors.border}` }}>
      <span style={{ fontSize: 11, color: colors.textMuted, fontFamily: 'var(--font-mono)', textTransform: 'uppercase' }}>{label}</span>
      <span style={{ fontSize: 12, color: colors.text, fontFamily: 'var(--font-sans)' }}>{value}</span>
    </div>
  );
}
